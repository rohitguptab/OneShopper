...
exports.importExamples = async () => {
    ...
    let contentTypeDefinitionResponse = await fetch(apiUrl + `/api/v1/internal/contenttype/product`, { headers: headers })
    let imageToReplace = []
    ...
    await Promise.all(files.map(async function (file) {
        if (file.indexOf(`contentObject`) === 0) {
            ...
            let res = await fetch(apiUrl + `/api/v1/content/product/` + id, { method: `HEAD`, headers: headers })
            let method = `POST`
            let url = apiUrl + `/api/v1/content/product`
            if (res.status === 403) {
                assert.ok(false, `Please provide correct API key (Read and write Application API key)`)
            }
            if (res.ok) {
